...
export function TodoList({ todos: initialTodos }: TodoListProps) {
  ...
  const todoStateCounts: Record<TodoState, number> = {
    active: 0,
    completed: 0,
  };
  ...
  const toggleTodo = async (id: string) => {
    ...
    try {
      await todosApi.updateTodo(id, { completed: newVal });
    } catch (err) {
  ...
  const editTodo = async (id: string, title: string) => {
    ...
    try {
      await todosApi.updateTodo(id, { title });
    } catch (err) {
  ...
  // Sync modal with path param /todos/:id/edit
  useEffect(() => {
    if (!editIdParam) {
    ...
    (async () => {
      try {
        const fetched = await todosApi.getTodo(editIdParam);
  ...
  todoStateCounts.completed = todos.filter((todo) => todo.completed).length;
  todoStateCounts.active = todos.length - todoStateCounts.completed;
